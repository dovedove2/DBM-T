...
local warnRagingStorms				= mod:NewCastAnnounce(192696, 4)
local warnCrushingDepths			= mod:NewTargetAnnounce(197365, 4)
...
local yellCrushingDepths			= mod:NewYell(197365)

local timerMythicTornadoCD			= mod:NewCDTimer(25, 192680, nil, nil, nil, 3)
local timerMassiveDelugeCD			= mod:NewCDTimer(54, 192617, nil, "Tank", nil, 5, nil, DBM_CORE_TANK_ICON)
local timerArcaneBombCD				= mod:NewCDTimer(30, 192706, nil, nil, nil, 3)--23-37
local timerCrushingDepthsCD			= mod:NewCDTimer(40, 197365, nil, nil, nil, 3, nil, DBM_CORE_DEADLY_ICON)

mod:AddRangeFrameOption(10, 192706)

mod:AddSetIconOption("SetIconOnCrushingDepths", 197365, true, false, {8})
...
function mod:OnCombatStart(delay)
	...
	timerArcaneBombCD:Start(30-delay)
	timerCrushingDepthsCD:Start(20-delay)
end
...
function mod:SPELL_CAST_START(args)
	...
	if spellId == 192985 then
		self.vb.phase = 2
		timerMythicTornadoCD:Start(22)
		timerMassiveDelugeCD:Start(41)
		timerArcaneBombCD:Start(26.2)
		timerCrushingDepthsCD:Start(48.5)
