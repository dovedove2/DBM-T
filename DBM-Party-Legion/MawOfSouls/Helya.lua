...
local timerBrackwaterBarrageCD			= mod:NewCDTimer(23, 202088, nil, nil, nil, 2, nil, DBM_CORE_DEADLY_ICON)
...
function mod:OnCombatStart(delay)
	self.vb.phase = 1
	timerPiercingTentacleCD:Start(10-delay)
	timerBrackwaterBarrageCD:Start(20-delay)
	countdownBrackwaterBarrage:Start(20-delay)
	timerTaintofSeaCD:Start(5.3-delay)
end
...
function mod:SPELL_CAST_START(args)
	...
	elseif spellId == 202088 then
		...
		countdownBrackwaterBarrage:Start()
		--timerBreathCD:Start()
		--countdownBreath:Start()
...
function mod:SPELL_AURA_REMOVED(args)
	local spellId = args.spellId
	if spellId == 196947 then --these timers for first submerge, change for next ones (To do)
		timerTorrentCD:Start(29)
